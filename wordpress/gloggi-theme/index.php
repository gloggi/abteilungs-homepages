...
$recaptcha = wpptd_get_post_meta_value($post->ID, 're-captcha');
if ($recaptcha) {
...
if (isset($_POST['submit'])) {
    ...
    if($recaptcha){
        ...
        $data = array(
            'secret' => wpod_get_option( 'gloggi_einstellungen', 're-captcha-secret-key'),
            'response' => $_POST["g-recaptcha-response"]
        );
        $options = array(
            'http' => array (
        ...
        <form action="<?php echo get_the_permalink(); ?>#kontakt" method="POST">
            <ul>
                ...
                foreach ($formfields as $field) : ?>
                    <li><label
                            for="field<?php echo $index; ?>"><?php echo $field['name'] . ($field['required'] ? '*' : ''); ?></label>
                    <?php if ($field['type'] == 'textarea') : ?>
                        <textarea rows="3" name="field<?php echo $index; ?>" id="field<?php echo $index; ?>"
                                  <?php if ($field['required']) : ?>required="required"
                                  <?php endif; ?>class="<?php echo $field['class']; ?>"><?php echo $prefill[$index]; ?></textarea></li>
                    <?php elseif ($field['type'] == 'gender') : ?>
                        <select name="field<?php echo $index; ?>" id="field<?php echo $index; ?>"
                                <?php if ($field['required']) : ?>required="required"
                                <?php endif; ?>class="<?php echo $field['class']; ?>">
                            <option value="">Bitte w&auml;hlen</option>
                            <option value="m"<?php if ($prefill[$index] == 'm') echo ' selected="selected"'; ?>>m
                            </option>
                            <option value="w"<?php if ($prefill[$index] == 'w') echo ' selected="selected"'; ?>>w
                            </option>
                            <option value="x"<?php if ($prefill[$index] == 'x') echo ' selected="selected"'; ?>>x
                            </option>
                        </select>
                    <?php elseif ($field['type'] == 'clothing-size') : ?>
                        <select name="field<?php echo $index; ?>" id="field<?php echo $index; ?>"
                                ...
                                <?php endif; ?>class="<?php echo $field['class']; ?>">
                            <option value="">Bitte w&auml;hlen</option>
                            <option value="XS"<?php if ($prefill[$index] == 'XS') echo ' selected="selected"'; ?>>XS
                            </option>
                            <option value="S"<?php if ($prefill[$index] == 'S') echo ' selected="selected"'; ?>>S
                            </option>
                            <option value="M"<?php if ($prefill[$index] == 'M') echo ' selected="selected"'; ?>>M
                            </option>
                            <option value="L"<?php if ($prefill[$index] == 'L') echo ' selected="selected"'; ?>>L
                            </option>
                            <option value="XL"<?php if ($prefill[$index] == 'XL') echo ' selected="selected"'; ?>>XL
                            </option>
                        </select>
                    <?php elseif ($field['type'] == 'date') : ?>
                        <input name="field<?php echo $index; ?>" id="field<?php echo $index; ?>"
                               value="<?php echo $prefill[$index]; ?>"
                               <?php if ($field['required']) : ?>required="required"
                               <?php endif; ?>class="datepicker <?php echo $field['class']; ?>"/>
                    <?php else : ?>
                        <input type="<?php echo $field['type']; ?>" name="field<?php echo $index; ?>"
                               id="field<?php echo $index; ?>" value="<?php echo $prefill[$index]; ?>"
                               <?php if ($field['required']) : ?>required="required"
                               <?php endif; ?>class="<?php echo $field['class']; ?>"/>
                    <?php endif; ?>
                    </li>
                ...
                <?php if ($recaptcha): ?>
                    <li>
                        <div class="g-recaptcha" style="margin-top: 10px;"
                             data-sitekey="<?php echo wpod_get_option( 'gloggi_einstellungen', 're-captcha-site-key'); ?>"></div>
                    </li>
                <?php endif; ?>
            </ul>
